  ...
  this.projects = [
    {
      ...
      target: 7,
      people: [],
      bugs: ["651501 document.body properties fail to autocomplete", "634406 Select All unexpectedly results in multi-line input", "653110 undefined appended after pasted text"]
    }, {
      id: "highlighter",
      ...
      bugs: ["--- Blockers ---", "642471 Rewrite PanelHighlighter using transparent xul iframe", "666650 global toolbar", "650802 Create some controller UI", "650794 Strip HTML panel and support code from browser", "653528 Strip out Style and DOM panels and support code from Inspector", "653531 Shared knowledge of selected node", "653549 Add a close button to the selected node", "653550 Add a close button", "587134 Context menu item for Highlight Element", "566084 disabled when navigating to new pages", "566085 tracking DOM changes", "660606 allow registration of developer tools", "663100 Re-implement highlighting mechanism in HTML+flexbox", "663781 Once a node is locked, the black background should be removed", "663778 Draw layout information of the selected node", "663834 registerTools() should allow tools to be docked in the browser", "663778 Draw layout information", "672902 usable via keyboard", "672876 Inspect panel opens up on the wrong monitor", "--- Want ---", "650825 create a content broker", "672006 Selecting a node should update toolbar with a breadcrumb", "672003 Togglable classes in the infobar", "672002 dock the HTML panel", "671689 Nodes should be selectable from the HTML tree", "671294 Opening the Inspect window with the Web Console open causes the panel to open in the wrong position", "671291 iframe and html overlap in the inspect window", "674871 can't close inspect", "669652 Move the highlighter the code from inspector.js to a jsm", "668435 hilight box should round corners to match", "668254 doesn't handle full page zoom correctly", "--- Now You're Showing Off ---", "653545 Create a means to highlight multiple items on a single webpage", "665539 find the best duration and delay for transitions", "653534 Create basic Annotations feature", "663852 support transformed elements (CSS3 transform)"]
    }, {
      id: "styleinspector",
      ...
      target: 8,
      people: ["mratcliffe"],
      bugs: ["582596 Style view answering common CSS questions", "663831 Should be controllable from the Highlighter", "586974 All unmatched rules are ranked the same", "669596 Style Inspector should allow editing of css property values", "593331 Csslogic shortSource can create duplicate names", "583037 Better logic for placement of inspector panels", "590536 Inspector panels should remember at least size and ideally also position", "587752 Animations in the new Style panel", "585563 The inspector style panel should link to the CSS editor", "590796 Clicking on a rule doesn't jump to the correct line in the CSS editor", "589264 Add option to choose an alternative media type", "589375 Keyboard access", "591212 React to dynamic changes to the stylesheets", "591584 Localize the URLs in csshtmltree", "591902 Automatic display of localized content", "593345 Investigate an advanced test tool for the inspector style panel", "656220 Create styleInspector obj. containing cssLogic & cssHtmlTree, open/close methods", "656027 It should be possible to pref out the Style Inspector", "574347 Opening the HUD with Inspector open causes Style Panel to move", "586977 Investigate how the panel works with shorthand properties", "588941 Computed style for property is sometimes wrong", "591582 Style Panel CSS properties are limited to hard coded values", "652509 Style inspector should skip unsupported styles", "653082 Stylesheet selector should be removed", "653084 Specificity should be removed", "654068 Show visual indicator when there are unmatched rules but not matched rules", "654069 \"Effects and Other\" group should be extracted into more than 1 group", "654430 Unmatched rules do not work for string values", "672806 Style Inspector should be docked on the right of the content area", "672748 UI refresh", "672746 'include default styles' checkbox", "672744 Add search filter", "672743 remove category view", "674485 missing color legend for its rule colors", "674482 Make header fixed to ease knowing which element is being inspected", "669486 Uses same template engine as GCLI but different copy"]
    }, {
      id: "devtoolbar",
      ...
      status: "Planning",
      people: "paul",
      bugs: ["674476 Sidebar to host the developer tools"]
    }, {
      id: "gcli",
      ...
      status: "Implementation",
      people: ["msucan"],
      bugs: ["660784 Add a source code editor to the browser", "668320 Integrate Orion unit and performance tests"]
    }, {
      id: "cssedit",
      name: "Style Editor",
      ...
      target: 8,
      people: ["cedricv"],
      bugs: ["583041 CSS editor", "590307 inline stylesheet support", "590795 edits should be reflected in the Inspector", "590796 style inspector link to the CSS editor", "590797 resizable editor panel", "590799 CSS editor styling", "594742 unsaved state reappears when the editor is reopened", "674177 use Orion", "673129 import a file as a new style sheet", "673127 save to file", "671350 responive UI"]
    }, {
      id: "sourcemap",
      name: "SourceMap",
      ...
      status: "Implementation",
      people: ["fitzgen"],
      bugs: ["618650 Map JS source coordinates to source language that was translated to JS", "674283 Add sourcemap URLs to JSScript", "673487 Add ability to map from original to generated source to SourceMapConsumer", "672312 Mini-require should be pulled out of GCLI so it can be shared with Source Map", "670002 use sourcemaps in the web console", "669999 Add a library for parsing and consuming source map files"]
    }, {
      id: "tilt",
      name: "Tilt",
      ...
      status: "Planning",
      people: [],
      bugs: ["672470 Fix memory leaks", "644596 expand console object with missing methods", "658368 Expand console object with time and timeEnd methods", "659625 Expand console object with clear method", "659907 Expand console object with dir method", "664131 Expand console object with group methods", "614586 Implement string substitution in console API methods", "669861 Cached errors and messages have wrong time stamp", "609890 Errors from before console is opened don't appear", "611032 Break out HUDConsoleObserver from HUDService", "612658 Implement ConsoleStorageService", "612252 Clear console keyboard shortcut", "655700 More visual separation between requests", "656231 cleanup the HUD object properties", "656709 Net toggle isn't tab-specific", "657932 lack of symmetry in handling of enter and escape keys", "592552 History is shared among all Web Console instances", "622303 Web Console should remember filter settings", "649350 Cleanup/reorganize the HUDService observers and listeners", "630460 command-w doesn't close inspector panels", "616586 Internal console logging API", "588871 Remove ConsoleStorage methods used for log message storage", "646816 command-w binding for dismissing panels", "656461 Web Console calls getters when displaying an object", "659931 Clicking on position button changes position", "660910 Autocompletion doesn't support this.x", "660864 completeNode is accepting keyboard input", "659775 command-w to close separate Web Console window", "663151 Display IP address and IP version in network panel", "663366 Full-screen web console is missing toolbar", "662807 Error bubbles in multiline messages in console look silly", "638949 Copy Location for URLs", "646028 Add debug filter item to the toolbar", "674229 Repeat bubbles are too close to source link", "673891 Convert message items to XBL", "673148 e10s", "670897 add inner windowID and timestamp to nsiScriptError2", "670857 deliberate uncaught exceptions will cause test failures in the future", "670140 browser_webconsole_bug611795.js leaks its content window", "664568 Allow ConsoleAPIStorage to keep logged messages in PB Mode", "664688 fails to track back/forward", "664466 sometimes hud is undefined in HS_deactivateHUDForContext", "663477 Can't close the Web Console window by repeating the keyboard shortcut: both windows are inactive", "662611 Console messages can freeze browser or tab"]
    }, {
      id: "scratchpad9",
